...
To create a new visualization you need to create a new visualization type using the VisualizationFactory.
...
The base visualization type does not make any assumptions about the rendering technology and
works with pure Javascript. It's the recommended visualization type.

You need to provide an object with a constructor, render function, and destroy function. The render function will be called every time options or data changes. The render function is called with the data object and status object. The status object contains a set of boolean properties identifying which components have changed. The render function needs to return a promise, which should be resolved once the visualization is done rendering. The destroy function will be called to cleanup.
...
the old data). Implementing request handler is optional, you might use one of the provided.
- response from `requestHandler` will get passed to `responseHandler`. IT should convert raw data to something that
can be consumed by visualization. Implementing responseHandler is optional, you might use one of the provided.
...
AngularJS visualization type uses angular for rendering. Configure VisFactory with an angular template.

Visualization will receive vis, uiState, visData, and renderComplete on $scope. Execute renderComplete when rendering is complete.
...
React visualization type uses React for rendering. Configure VisFactory with a React component.

Visualization will receive vis, uiState, visData, and renderComplete on props. Execute renderComplete when rendering is complete.
